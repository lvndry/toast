...
3. Updates all 'company_slug' fields to 'product_slug' in meta_summaries and deep_analyses collections
"""
...
DATABASE_NAME = "toast"  # TODO: Change to clausea
...
async def migrate_companies_to_products():
    ...
    try:
        ...
        # Step 3: Update company_slug to product_slug in meta_summaries
        logger.info("Updating 'company_slug' to 'product_slug' in meta_summaries collection...")
        result = await db.meta_summaries.update_many(
            {"company_slug": {"$exists": True}},
            {"$rename": {"company_slug": "product_slug"}},
        )
        logger.info(f"✓ Updated {result.modified_count} meta_summaries")
        ...
        # Step 5: Update company_id references in meta_summaries and deep_analyses if they exist
        logger.info("Checking for company_id references in meta_summaries...")
        result = await db.meta_summaries.update_many(
            {"company_id": {"$exists": True}},
            {"$rename": {"company_id": "product_id"}},
        )
        if result.modified_count > 0:
            logger.info(f"✓ Updated {result.modified_count} meta_summaries with company_id")
