...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!companyId && !domain) {
      return NextResponse.json({ error: "Either companyId or domain parameter is required" }, { status: 400 });
    }

    // If we have a company ID, fetch the company from backend
    if (companyId) {
      const response = await fetch(`${BACKEND_BASE_URL}/toast/companies/${companyId}`);
      if (response.ok) {
