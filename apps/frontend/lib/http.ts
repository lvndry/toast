...
export interface HttpOptions<TBody = unknown> extends Omit<RequestInit, 'body'> {
  retry?: number;
  retryDelayMs?: number;
  log?: boolean;
  body?: TBody;
...
export async function http(
  ...
  const { retry = DEFAULT_RETRY, retryDelayMs = DEFAULT_RETRY_DELAY_MS, log = true, headers, ...init } = options;
